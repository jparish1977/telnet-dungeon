...
A multiplayer dungeon crawler you play over telnet. Pure Python 3 with minimal dependencies.
...
# Start the telnet server (default port 2323)
python dungeon_server.py

# Or specify a custom port
python dungeon_server.py 8080
...
- **Player Trading System** — Drop/pick up gold, trade 1-on-1 with other players via `E` key
...
- Pure Python 3 with minimal dependencies (`websockets` for web server)
...
**New Module:** `dungeon/trading.py` (450+ lines)
- Drop gold on ground (persists for other players)
- Pick up gold (from anywhere on tile)
- Trade directly with players on same tile (1-on-1)
- Real-time player discovery
- Broadcast notifications create social economy

**How to Use:**
- Press `E` (Exchange) to open trading hub mid-game
- `[D]` Drop gold — leaves inventory, appears on ground
- `[P]` Pick up gold — collect from shared piles  
- `[T]` Trade — select target, propose amount, await response (20-second timeout)

**Economic Features:**
- Persistent ground gold (doesn't despawn, can be used as storage)
- Broadcast visibility (trades create public messages)
- Enables mentorship (veterans gift to newbies)
- Faction warfare (pool resources for group goals)
- Creates natural gathering points in towns

**Integration:**
- Modified `dungeon/session.py` — added import, command handler, UI display
- Uses existing `World` object for player discovery
- Integrates with existing save mechanisms

### Extensive Lore Expansion 📖

**Total New Content:** 12+ new markdown files with 10,000+ words of story content

#### Major Quest Lines (6 Quests)

**Existing:**
- `stolen_mower.md` — Humorous quest about a farmer's magical device (comedic/light)
- `bookeater_gyre.md` — Morally complex quest with an intelligent creature (moral dilemma)

**New:**
- `forgotten_archive.md` — Uncover suppressed truth about The Sundering (conspiracy, 1200+ words)
- `cartographers_maps.md` — Retrieve legendary maps from flooded dungeon (exploration, 1200+ words)
- `poisoned_well.md` — Investigate dimensional breach affecting local water (mystery/horror, 1000+ words)
- `deepest_dark.md` — Journey to absolute dungeon depths (cosmic horror/epic, 1500+ words)
...
**New File:** `stories/factional_politics.md` (1500+ words)
- 6 competing factions with hidden agendas:
  - **The Orthodox Codex** — Authoritarian knowledge keepers
  - **The Underground** — Revolutionary resistance movement
  - **Church** — Mystical faith-based organization
  - **Deepwarriors** — Underground dweller clans
  - **Shadow Market** — Merchants of forbidden goods
  - **Wardens** — Dimensional boundary enforcers
- Factional tensions and player implications
- Quest alignment matrices
- Hidden agendas reveal themselves through gameplay

#### Regional Lore

**New File:** `stories/regions/niles.md` (1000+ words)
- The Frontier town with mysterious chasm
- The Deep Watch (unknown purpose)
- Gravity well and chasm echo mechanics
- Rumors about metallic objects awakening

**Existing Regions (organized):**
- Buchanan — The Gateway, hub city
- Benton — The Harbor, herbalist center  
- Coloma — Quest Vault, scholarly hub

#### Game Systems Documentation

**New Files:**
- `stories/LORE_MAP.md` — Master document showing all connections, quest relationships, progression paths
- `stories/TRADING_GUIDE.md` (2,100+ words) — Complete guide to trading system, economy mechanics, strategic uses, best practices, FAQ

#### Enhanced Navigation

**Updated:** `stories/README.md`
- Reorganized with comprehensive site map
- Quick start guides for different playstyles
- Theme summary
- Links to all story content

### Story Themes

| Layer | Focus | Quests |
|-------|-------|--------|
| **Surface (Adventure)** | Explore, fight, get treasure | Stolen Mower, Bookeater Gyre |
| **Intermediate (Mystery)** | What is The Sundering? Origins? | Cartographer's Maps, Poisoned Well |
| **Deep (Politics)** | Which faction to trust? | Forgotten Archive, Factional Quests |
| **Deepest (Cosmic)** | What is The Keeper? What am I? | Deepest Dark (intentionally open-ended) |

### Story Connections

- Quests can be solved multiple ways depending on faction alignment
- Trading system enables "black market" solutions
- Pool gold with allies for expensive faction rewards
- Lore progression reveals through exploration and NPC dialogue
- Each playthrough can emphasize different thematic layers

## Code Quality

Run the code quality checker to analyze your codebase:

```bash
# Basic analysis
python quality/check_code_quality.py

# Verbose output (all warnings)
python quality/check_code_quality.py --verbose

# Export results as JSON
python quality/check_code_quality.py --json
```

The checker provides:
- Syntax error detection
- PEP 8 style checking (tabs, trailing whitespace, line length)
- Module import analysis
- Code statistics (files, lines, functions, classes)
- Quality metrics and recommendations

**Current Status:** ✅ 0 syntax errors across all modules
...
telnet-dungeon/
  ...
  README.md                       # Project documentation with recent updates
  dungeon/                       # Game engine package (19 modules)
    ...
    trading.py                   # Player trading system (NEW)
    ...
    protocol/
      ...
      telnet.py                  # Telnet I/O (IAC, WILL/DO, SB/SE, NAWS, ANSI)
      stdio.py                   # Local terminal adapter (--local mode)
    gm/
      tools.py                   # GM tools (player admin, editors)
  quality/                       # Code quality tools
    check_code_quality.py        # CLI code quality checker with PEP 8 analysis
    config.py                    # Constants, ANSI colors, tile codes
    items.py                     # Weapons, armor, spells, character classes
    persistence.py               # All JSON file I/O (saves, bans, themes)
    floor.py                     # Dungeon generation, overworld, caching
    ...
    gm/
      tools.py                   # GM tools (player admin, editors)
  stories/                       # Game lore and quest documentation
    README.md                    # Story navigation hub
    LORE_MAP.md                  # Master connections document
    TRADING_GUIDE.md             # Trading system guide and economy
    factional_politics.md        # 6 factions and hidden agendas
    world_history.md             # The Sundering and emergence
    timeline.md                  # Chronological history
    npcs_and_factions.md         # Key NPCs and organizations
    monsters_lore.md             # Creature bestiary
    quests/
      stolen_mower.md            # Farmer's comedy quest
      bookeater_gyre.md          # Moral complexity with intelligent creature
      forgotten_archive.md       # Conspiracy to uncover hidden truth
      cartographers_maps.md      # Retrieve maps from flooded dungeon
      poisoned_well.md           # Investigate dimensional breach
      deepest_dark.md            # Epic quest to absolute depths
    regions/
      buchanan.md                # The Gateway, hub city
      benton.md                  # The Harbor, herbalist center
      coloma.md                  # Quest Vault, scholarly hub
      niles.md                   # The Frontier with mysterious chasm
  saves/                         # Character save files (JSON)
  custom_floors/                 # GM-edited floor overrides (JSON)
  quests/                        # Quest definitions (JSON)
  region_maps/                   # Regional map data (JSON)
