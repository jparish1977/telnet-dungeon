...
export class TerminalRenderer {
  ...
  private initGrid() {
    ...
    for (let r = 0; r < this.rows; r++) {
      ...
      for (let c = 0; c < this.cols; c++) {
        row.push({ char: " ", fg: DEFAULT_FG, bg: "" });
      }
      this.grid.push(row);
    }
  }
  ...
  private resize() {
    ...
    const fontSize = Math.max(MIN_FONT_SIZE, Math.floor(rect.height / this.rows));
    this.ctx.font = `${fontSize}px "Courier New", monospace`;
    const metrics = this.ctx.measureText("M");
    this.cellW = metrics.width;
    this.cellH = fontSize * LINE_HEIGHT_RATIO;
  ...
  private processEscape(text: string, start: number): number {
    let i = start;
    let code = "";
    while (i < text.length && text[i] !== "m" && text[i] !== "H" && text[i] !== "J" && text[i] !== "K") {
      code += text[i];
      i++;
    }
  ...
  render() {
    ...
    const fontSize = Math.max(MIN_FONT_SIZE, Math.floor(rect.height / rows));
    ...
    for (let r = 0; r < rows; r++) {
      for (let c = 0; c < cols; c++) {
        ...
        if (cell.char !== " ") {
          ...
          ctx.fillText(cell.char, x, y + TEXT_Y_OFFSET);
        }
      }
    }
  }
