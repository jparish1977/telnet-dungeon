...
async def gm_pick_player(session, world, prompt="Pick player: "):
    ...
    for i, (name, s) in enumerate(players, 1):
        ...
        await session.send_line(f"  {color(f'[{i}]', YELLOW)} {name} Lv.{s.char['level'] if s.char else '?'} {loc}{gm_tag}")
        ...
        if choice == 'B':
            ...
            if target and target.char:
                ...
                if stat == '9':
                    ...
                    fields = keys[stat]
                    for field in fields:
                        val = await session.get_input(f"  {field} = ")
                        try:
                            c[field] = int(val)
                        except ValueError:
                            pass
            ...
            if target and target.char:
                await session.send_line(f"  Current: Floor {target.char['floor']+1} ({target.char['x']},{target.char['y']})")
                fl = await session.get_input("  Floor (0+): ")
                x = await session.get_input("  X: ")
                y = await session.get_input("  Y: ")
                try:
            ...
            if target and target.char:
                ...
                reason = await session.get_input("  Reason: ")
                if not reason:
                    reason = "Kicked by GM"
            ...
            for name, s in world.sessions.items():
                if s.char:
                    ...
                    await session.send_line(f"    {name} Lv.{c['level']} {c['class']} F{c['floor']+1} ({c['x']},{c['y']}) HP:{c['hp']}/{c['max_hp']} Gold:{c['gold']}{gm}")
            await session.send_line()
            ...
            for sname in list_saves():
                if sname not in [n.lower() for n in world.sessions]:
                    ...
                    if sc:
                        ...
                        await session.send_line(f"    {sc['name']} Lv.{sc['level']} {sc['class']} F{sc['floor']+1}{banned}")
            if world.banned:
                await session.send_line()
...
async def edit_art_lines(session, current_art=None):
    ...
    while True:
        ...
        await session.send_line()
        await session.send_line(f"  {color('A', YELLOW)}dd  {color('E', YELLOW)}dit#  {color('D', YELLOW)}el#  {color('I', YELLOW)}ns#  {color('P', YELLOW)}lot(r,c,ch)  {color('R', YELLOW)}eplace  {color('Q', YELLOW)}done")
        ...
        if cmd == 'q':
            ...
            num = await session.get_input("  line #: ")
            try:
                idx = int(num) - 1
                if 0 <= idx < len(art):
            ...
            num = await session.get_input("  del #: ")
            try:
                idx = int(num) - 1
                if 0 <= idx < len(art):
                    art.pop(idx)
            ...
            num = await session.get_input("  insert before #: ")
            try:
                idx = int(num) - 1
                if 0 <= idx <= len(art):
                    line = await session.get_input("  new line> ", preserve_spaces=True)
            ...
            r_inp = await session.get_input("  row: ")
            c_inp = await session.get_input("  col: ")
            ...
            try:
                row = int(r_inp) - 1
                col = _parse_col(c_inp) if not c_inp.isdigit() else int(c_inp) - 1
                while len(art) <= row:
                    art.append("")
                if len(art[row]) <= col:
            ...
            new_art = []
            while True:
                line = await session.get_input("  art> ", preserve_spaces=True)
                if not line:
                    break
                new_art.append(line)
            if new_art:
    ...
    while True:
        ...
        if ch == 'B':
            ...
            for i, (fl, m) in enumerate(all_builtins):
                await session.send_line(f"  {color(f'[{i+1:2d}]', YELLOW)} F{fl+1} {color(m['name'], WHITE):20s} HP={m['hp']:3d} ATK={m['atk']:2d} DEF={m['def']:2d} XP={m['xp']:3d} G={m['gold']}")
                art = m.get('art') or default_arts.get(m['name'], ["  [?_?]"])
                for aline in art:
                    await session.send_line(color(f"       {aline}", RED))
            await session.send_line(f"\n  {color('[0]', YELLOW)} Back")
            pick = await session.get_input("  Edit #: ")
            try:
                idx = int(pick) - 1
            ...
            await session.send_line(f"  HP={hp} ATK={atk} DEF={dfn} XP={xp} Gold={gold} Floor={'ALL' if fl==-1 else fl+1}")
            if art_lines:
            ...
            for i, m in enumerate(customs):
                ...
                await session.send_line(f"  {color(f'[{i+1}]', YELLOW)} {color(m['name'], WHITE)} HP={m['hp']} ATK={m['atk']} DEF={m['def']} XP={m['xp']} G={m['gold']} ({fl_str})")
                art = m.get('art', [])
                if art:
                    for aline in art:
                        await session.send_line(color(f"       {aline}", RED))
...
async def gm_viewport_theme_editor(session):
    ...
    while True:
        await session.send(CLEAR)
        await session.send_line(color(f"=== VIEWPORT THEME - Floor {floor+1 if floor >= 0 else 'Overworld'} ===", MAGENTA))
        await session.send_line()
...
async def gm_scene_editor(session, world):
    ...
    while True:
        ...
        if cmd == 'q':
            ...
            await session.move_to(1, 1)
            await session.send("\033[2K")
            new_size_str = await session.get_input(f" New size (current {size}, max 256): ")
            try:
